import {useState} from 'react';
const AdminDeletePharmacist = () => {

const [username,setUsername]=useState("");
const [error,setError]=useState(null);

    const handleClick = async (e) => { 
        e.preventDefault(); //prevent default action of submitting
        const pharmacist = {username}; //body of req
        console.log("pharmacist to be deleted: "+ pharmacist)
        const response = await fetch(`/api/admin/deletePharmacistByUsername/${pharmacist.username}`, // + admin._id?
        {
            method: 'DELETE'
        })
        const json = await response.json();
        console.log("deleted pharmacist: "+ json)
        
        
    }
       



    return (
        <form className="delete" onSubmit={handleClick}>
            <h3> Delete a Pharmacist</h3>
            <label>Username</label>
            <input 
              type="text"
              onChange={(e)=> setUsername(e.target.value)}
              value={username}
            />
            <button>Delete Pharmacist</button>
            {error && <div className="error">{error}</div>}
        </form>
        
    ) 

}
export default AdminDeletePharmacist